...
from flask import Flask, abort, make_response, render_template, url_for, redirect
...
from openslide import OpenSlide, OpenSlideError, ImageSlide, open_slide
...
class _SlideCache:
    ...
    def get(self, path):
        ...
        #############
        print(osr.associated_images)
        app.slides = {'slide': DeepZoomGenerator(osr, **self.dz_opts)}
        slide.properties = osr.properties
        app.associated_images = []
        for name, image in osr.associated_images.items():
            app.associated_images.append(name)
            slug = slugify(name)
            app.slides[slug] = DeepZoomGenerator(ImageSlide(image), **self.dz_opts)
        print(app.slides)
        ##############
        try:
...
def _setup():
    ...
    app.files = [p.url_path for p in _Directory(app.basedir).children]
    app.current_slide = 0
    config_map = {
...
@app.route('/next_slide', methods=['POST', 'GET'])
def next_slide():
    ind = (app.current_slide+1) % (len(app.files))
    return redirect(app.files[ind])

@app.route('/prev_slide', methods=['POST', 'GET'])
def prev_slide():
    ind = (app.current_slide+-1) % (len(app.files))
    return redirect(app.files[ind])
...
def slide(path):
    print(path)
    app.current_slide = app.files.index(path)
    print(app.current_slide)
    slide = _get_slide(path)
    slide_url = url_for('dzi', slug=slugify('slide'))
    associated_urls = {
...
#     except:
#         print(app.slides[path])
#         resp = make_response(app.slides[path].get_dzi(format))
